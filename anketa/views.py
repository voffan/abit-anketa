...
def GetAbiturient(request):
	...
	if appTypes is not None:
		if len(appTypes) > 0:
			...
			appProf = ApplicationProfiles.objects.filter(profile__profile__id=request.GET['eduprof'])
			kostil=[]
			...
			for appT in appTypes:			
				...
				appProfId=[]
				for prof in appProf:
					if prof.application.id in appAttrId:
						#kostil.append(prof.application.id)
						appProfId.append(prof)

				#print("appAttrId ",appT.value, appAttrId)				
				#print("appProf ",appProf)
				abitId = [x.application.abiturient.id for x in appProfId]
				...
				for abitur in abiturients:
					...
					print("appT_ID ",appT.id, "abit ",abitur.fullname)
					result.append({'abiturient':abitur.fullname,'points':points.get('points__sum'),'appType':appT.id})

			#print(">>>>>>>>>>>>>>",len(appProf))
			#appProf = appProf.exclude(application__id__in=kostil)
			#print("<<<<<<<<<<<<<<",len(appProf))
			abitId = [x.application.abiturient.id for x in appProf]
			abiturients = Abiturient.objects.filter(id__in=abitId)
			for abitur in abiturients:
				points = Exams.objects.filter(abiturient=abitur).aggregate(Sum('points'))
				print("obshak ", abitur.fullname)
				result.append({'abiturient':abitur.fullname,'points':points.get('points__sum'),'appType':'-1'})

	"""for item in abiturients:
		points = Exams.objects.filter(abiturient=item).aggregate(Sum('points'))
		result.append({'abiturient': item.fullname, 'points': points.get('points__sum'),'appType':'-1'})
	"""
