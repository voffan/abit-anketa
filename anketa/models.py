...
class Abiturient(Person):
	...
	citizenship = models.ForeignKey(AttrValue,verbose_name=u'Гражданство', limit_choices_to={'attribute__name':u'Гражданство'}, db_index = True,related_name='Citizenship', null=True, blank=True)
	foreign_lang = models.ForeignKey(AttrValue, verbose_name=u'Изучаемый иностранный язык', limit_choices_to={'attribute__name':u'Иностранный язык'}, related_name='Foreign', null=True, blank=True)
...
class Contacts(models.Model):
	...
	contact_type = models.ForeignKey('AttrValue',verbose_name=u'Тип контакта', limit_choices_to={'attribute__name':u'Тип контакта'}, related_name="ContactTypeAnketa")
...
class EduOrg(models.Model):
	...
	orgtype=models.ForeignKey('AttrValue', verbose_name=u'Тип образовательного учреждения', db_index=True)
	def __str__(self):
		return self.name
...
class Education_Prog(models.Model):
	...
	duration=models.ForeignKey('AttrValue', verbose_name=u'Срок обучения', limit_choices_to={'attribute__name':u'Срок обучения'}, db_index=True)
...
class Template(models.Model):
	...
	type=models.ForeignKey('AttrValue', verbose_name=u'Тип шаблона', limit_choices_to={'attribute__name':u'Тип шаблона'}, db_index=True)
	def __str__(self):
		return self.name
...
-class ApplicationProfiles(models.Model):
	...
	profile = models.ForeignKey(ProfileAttrs, verbose_name=u'Профиль направления', blank=True, null=True)
	def __str__(self):
